...
def evaluate_and_assign_folder(function_to_run, *args, folder_generator=None, input_space=None, **kwargs):
    if folder_generator is None:
        timestr = time.strftime("testrun %Y-%m-%d %H-%M")
        folder = os.path.join(data_folder, timestr)
    else:
        folder = os.path.join(data_folder, folder_generator(*args, **kwargs))
